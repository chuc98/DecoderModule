...
def decodes(code,advancekey):
	...
	while length_iterator < len(code_upper):
		...
		if char_to_explore.islower():
					char_to_explore=char_to_explore.upper()
		...
		if char_to_explore.isalpha():
			...
			decoded_string = decoded_string + list_alphabet[decode_index]
													    ...
													    decoded_index = int(char_to_explore) + int(advancekey)
													    if decoded_index > len(list_number):
																												decoded_index=decoded_index-int(len(list_number))
													    decoded_string = decoded_string + list_number[decoded_index]
